 ...
 *  @since      v5.0
 */
void SCCB_GPIO_init_1(void)
{
    gpio_init  (SCCB_SCL_1, GPO, 1); //初始化SCL
    gpio_init  (SCCB_SDA_1, GPO, 1); //初始化SDA

    port_init_NoALT(SCCB_SCL_1,ODO | PULLUP);
    port_init_NoALT(SCCB_SDA_1,ODO | PULLUP);
}
 ...
 *  @since      v5.0
 */
static uint8 SCCB_Start_1(void)
{
    SDA_H_1();
    SCL_H_1();
    SCCB_DELAY();
 ...
 *  @since      v5.0
 */
static int SCCB_WaitAck_1(void)
{
    SCL_L_1();
    //SDA_H();
    SDA_DDR_IN_1();
 ...
 *  @since      v5.0
 */
static void SCCB_SendByte_1(uint8 SendByte)
{
    uint8 i = 8;
    while(i--)
    {
 ...
 *  @since      v5.0
 */
static int SCCB_ReceiveByte_1(void)
{
    uint8 i = 8;
    uint8 ReceiveByte = 0;
...
* 注意  ：无
*****************************************************************************************/
static int SCCB_WriteByte_one_1( uint16 WriteAddress , uint8 SendByte );


int SCCB_WriteByte_1( uint16 WriteAddress , uint8 SendByte )            //考虑到用sccb的管脚模拟，比较容易失败，因此多试几次
{
    uint8 i = 0;
    while( 0 == SCCB_WriteByte_one_1 ( WriteAddress, SendByte ) )
    {
        i++;
        if(i == 20)
        {
...
int SCCB_WriteByte_one_1( uint16 WriteAddress , uint8 SendByte )
{
    if(!SCCB_Start_1())
    {
        return 0;
    }
    SCCB_SendByte_1( DEV_ADR );                    /* 器件地址 */
    if( !SCCB_WaitAck_1() )
    {
        SCCB_Stop_1();
        return 0;
    }
    SCCB_SendByte_1((uint8)(WriteAddress & 0x00FF));   /* 设置低起始地址 */
    SCCB_WaitAck_1();
    SCCB_SendByte_1(SendByte);
    SCCB_WaitAck_1();
    SCCB_Stop_1();
    return 1;
}
 ...
 * 注意  ：无
 **********************************************************************************************************************/
static int SCCB_ReadByte_one_1(uint8 *pBuffer,   uint16 length,   uint8 ReadAddress);

int SCCB_ReadByte_1(uint8 *pBuffer,   uint16 length,   uint8 ReadAddress)
{
    uint8 i = 0;
    while( 0 == SCCB_ReadByte_one_1(pBuffer, length, ReadAddress) )
    {
        i++;
        if(i == 30)
        {
...
int SCCB_ReadByte_one_1(uint8 *pBuffer,   uint16 length,   uint8 ReadAddress)
{
    if(!SCCB_Start_1())
    {
        return 0;
    ...
    SCCB_SendByte_1( ReadAddress );           /* 设置低起始地址 */
    SCCB_WaitAck_1();
    SCCB_Stop_1();
    ...
    SCCB_SendByte_1( DEV_ADR + 1 );               /* 器件地址 */
