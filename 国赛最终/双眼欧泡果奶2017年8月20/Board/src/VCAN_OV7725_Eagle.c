...
void ov7725_eagle_port_init_1()
{
    ...
    dma_portx2buff_init(OV7725_EAGLE_DMA_CH_1, (void *)&PTC_B1_IN, (void *)ov7725_eagle_img_buff_1, PTB19, DMA_BYTE1,OV7725_EAGLE_DMA_NUM_1, DADDR_KEEPON);

    DMA_DIS(OV7725_EAGLE_DMA_CH_1);
    disable_irq(PORTB_IRQn);                        //关闭PTB的中断，场中断
    ...
    port_init(PTB18, ALT1 | IRQ_RISING  | PULLUP | PF);     //场中断，上拉，上降沿触发中断，带滤波
...
void ov7725_eagle_vsync_1(void) 
{
    ...
    if(ov7725_eagle_img_flag_1 == IMG_START)                   //需要开始采集图像
    {
        ...
        PORTB_ISFR = 19 <<  PT1;            //清空PCLK标志位，PCk信号来了

        DMA_EN(OV7725_EAGLE_DMA_CH_1);                  //使能通道CHn 硬件请求
        PORTB_ISFR = 19 <<  PT1;            //清空PCLK标志位
        
        DMA_DADDR(OV7725_EAGLE_DMA_CH_1) = (uint32)ov7725_eagle_img_buff_1;    //恢复地址

#else
        PORTB_ISFR = 19 <<  PT1;            //清空PCLK标志位
        dma_repeat(OV7725_EAGLE_DMA_CH_1, (void *)&PTC_B1_IN, (void *)ov7725_eagle_img_buff,OV7725_EAGLE_DMA_NUM_1);
#endif
    }
    else                                        //图像采集错误
    {
...
reg_s ov7725_eagle_reg_1[] =
{
    ...
    {OV7725_SDE          , 0x04},
    {OV7725_BRIGHT       , 0x00},
    {OV7725_CNST         , 0x19},//南航赛场0x25 白天最好阈值0x17//摄像头1  //信标摄像头
    {OV7725_SIGN         , 0x06},
    {OV7725_UVADJ0       , 0x11},
    {OV7725_UVADJ1       , 0x02},
...
{
    //DMA通道0初始化，PTA27触发源(默认上升沿)，源地址为PTB_B0_IN，目的地址为：IMG_BUFF，每次传输1Byte
    dma_portx2buff_init(OV7725_EAGLE_DMA_CH, (void *)&PTB_B0_IN, (void *)ov7725_eagle_img_buff, PTA27, DMA_BYTE1, OV7725_EAGLE_DMA_NUM, DADDR_KEEPON);
...
void ov7725_eagle_vsync(void) 
{
    ...
    if(ov7725_eagle_img_flag == IMG_START)                   //需要开始采集图像
    {
        ...
        PORTA_ISFR = 27 <<  PT1;            //清空PCLK标志位，PCk信号来了

        DMA_EN(OV7725_EAGLE_DMA_CH);                  //使能通道CHn 硬件请求
        PORTA_ISFR = 27 <<  PT1;            //清空PCLK标志位
        
        DMA_DADDR(OV7725_EAGLE_DMA_CH) = (uint32)ov7725_eagle_img_buff;    //恢复地址

#else
        PORTA_ISFR = 27 <<  PT1;            //清空PCLK标志位
        dma_repeat(OV7725_EAGLE_DMA_CH, (void *)&PTE_B0_IN, (void *)ov7725_eagle_img_buff,OV7725_EAGLE_DMA_NUM);
#endif
    }
    else                                        //图像采集错误
    {
...
reg_s ov7725_eagle_reg[] =
{
    ...
    {OV7725_SDE          , 0x04},
    {OV7725_BRIGHT       , 0x00},
    {OV7725_CNST         , 0x20},//摄像头0  晚上最好19 //偏振片加合适阈值可以排除黄线
    {OV7725_SIGN         , 0x06},
    {OV7725_UVADJ0       , 0x11},
    {OV7725_UVADJ1       , 0x02},
